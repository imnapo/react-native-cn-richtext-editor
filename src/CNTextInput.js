...
class CNTextInput extends Component {
    ...
    onSelectionChange = (event) => {
        ...
        //console.log("onSelectionChange > ", event.nativeEvent.selection, this.justToolAdded);
    ...
    handleChangeText = (text) => {
        ...
        //console.log('lenght', this.textLength);
        ...
        let upComing = null;
            ...
            if(textDiff >= 0)
            {
                ...
                //console.log(content[foundIndex].text);
    ...
    updateNewLine(content, index, itemNo) {
        ...
        if(prevTag === 'ol') {
            ...
            if(itemNo === 0) {
                ...
                newContent = this.changeToTagIn(newContent, isPrevList ? prevTag : 'body', index);

            }
            else if(itemNo === foundElement.len - 1) {
               
                
                console.log('2', itemNo);
    ...
    applyStyle(toolType){
            ...
            for (var i = 0; i < content.length; i++) {
                const {id, len, stype,tag, NewLine, text, styleList} = content[i];
                ...
                let to = indx;
                    ...
                    else if(end > from && end <= to)
                    {
                        if(start === end)
                        {
                            ...
                            //console.log('upc', upComing, {start, end}, toolType);
                            if(upComingAdded === false) {
        ...
        for (let i = index + 1; i < content.length; i++) {
            if(content[i].NewLine === true) {
                ...
                if(needBold === true && content[i].stype.indexOf('bold') == -1) {
                    ...
                    StyleSheet.flatten(this.convertStyleList(newStype));
                } 
                else if(needBold === false && content[i].stype.indexOf('bold') != -1)  {
