...
class CNRichTextEditor extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        // console.log(this.focusOnNextUpdate, this.selectionOnFocus, this.textInputs.length);
  ...
  insertImage(url) {
    ...
    Image.getSize(url, (width, height) => {
        ...
        if(newConents.length === index + 1) {
          //console.log('adding');
          focusIndex = index + 1;
          newConents = update(newConents, { $splice: [[index + 1, 0, 
              [
                
                  {
                    component: 'text', text:'', content: [
                        {
                            len:0, stype:[], tag: 'body', text: '', styleList: [{
                                fontSize: 25
                             }]
                        }
                    ]   
    ...
    //       contents: newConents,
    //       texts:  newTexts,
    //       //focusInputIndex: index + 1 ,
    //       //selection: {start: 0, end: 0}
    //   }, () => {
          
    //     try {
    //         let comp = this.state.texts[index + 1];

    //         if(comp.component === 'text' && comp.text.length > 0 )
    //         {
    //             this.turnOnJustToolOnFocus = true;
    //         }
    //         else {
    //             this.turnOnJustToolOnFocus = false;
    //         }

    //         //console.log('turnOnJustToolOnFocus',this.turnOnJustToolOnFocus);
  ...
  removeImage =(index) => {
    ...
    if(content.component === 'image') {
        ...
        //     contents: newConents,
        //     texts:newTexts,
        //     selection: { start: selectionStart + 1, end: selectionStart + 1}
        // }, () => {
        //     if(index > 0) {
                
                  
        //          this.turnOnJustToolOnFocus = true;
        //          const input = this.textInputs[index - 1];
        //          input.focus();
